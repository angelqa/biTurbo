...
biTurbo.proto.createSpriteSheet = function (ssObject){
	...
	var tempPositionY = 0;
	var tempFrame = 0;
	var temp_nameContainerDiv = ssObject.htmlElement +"_container";
	var temp_containerStyle = "width : "+ ssObject.width + "px; height : " + ssObject.height + "px;  overflow:hidden";
	var back_insertionPoint = this.insertionPoint;
	ssObject.framesArray = [];

	this.createDivElement(temp_nameContainerDiv,"", temp_containerStyle,true);
	this.insertionPoint = temp_nameContainerDiv;
	this.createDivElement(ssObject.htmlElement,ssObject.source,"");
	this.insertionPoint = back_insertionPoint;
...
biTurbo.proto.nextFrameSpriteSheet = function (context,ssObject,direction){
	...
	if(ssObject.framesToPlay>0){
		...
		document.getElementById(ssObject.htmlElement).style.transform= "translate(-" + ssObject.positionX + "px,-" + ssObject.positionY +"px)";
	}else{
		context.stopSpriteSheet(context,ssObject);
	}
};

biTurbo.proto.playSpriteSheet = function (ssObject,delay,initFrame,framesToPlay, direction){
	var context = this;
	...
	setTimeout(function(direction){
		ssObject.temporalInterval=setInterval (context.nextFrameSpriteSheet , ssObject.frameDuration , context , ssObject , direction);
	},delay*1000);
};

biTurbo.proto.stopSpriteSheet = function (context,ssObject){
	clearInterval(ssObject.temporalInterval);
	context.a_print("should be executed function "+ssObject.functionOnEnd);
...
biTurbo.proto.checkLoaded = function (context){
	if(typeof(Event) === "function") {
		context.readyEvent = new Event("loadedEnough");
	}else{ //IE fix
    	context.readyEvent = document.createEvent('Event');
    	context.readyEvent.initEvent("loadedEnough", true, true);
	}
