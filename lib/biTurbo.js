...
biTurbo.proto.createDivElement = function (name , fileNameString , style, excludeFromElements){
	...
	if(this.insertionPoint === null){
		this.log(" Error: You must define insertionPoint in HTML ");
		return;
	}
	...
	if(fileNameString === ""){ //solid color
		...
		temp_divEnding = '<img id="' + temp_img_name + '" src="' + fileNameString + '" onload="' + this.itemLoaded(this,temp_img_name) + ';" style="visibility: hidden ; display : block;" /></div>'; // hack to set the div dimensions to fit the image, probably to be replaced in a future by a more polite solution if an image preloader is added
...
biTurbo.proto.itemLoaded = function (context,item){
	if(context.alreadyStarted === false){
