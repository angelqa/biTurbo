...
biTurbo.proto.animate = function (element , keyFramesArray ){
	var i = 0;
	...
	if(keyFramesArray === undefined || keyFramesArray.length < 1 ){
		this.a_print("wrong animate sintax!");
		return;
	}
	//this.a_print("animate length1er elemento: "+keyFramesArray[0][0])
	element = this.getDomElement(element);	
	...
	for(i = 0 ; i < temp_keyFrameCount ; i++){
		...
		this.tweenClass(element , temp_className , temp_delay , temp_tween);
	}
};
