...
biTurbo.prototype.applyClassToAll = function (name){
	var i=0;
	var element;
	for(i=0 ; i<this.elementsCreated.length ; i++){
...
biTurbo.prototype.tweenClass = function (element , keyframe , tween , delay){
	delay=defaultParameter(delay,0);
	element=this.getDomElement(element);	
	...
	setTimeout(function(){
		//element.className = "";		
		this.clearClass(element)
		applyClass(element,temp_name)
		applyClass(element,keyframe)
		applyClass(element,temp_tween)
	},delay*1000);
}
...
biTurbo.prototype.createClass = function (name,style){
	...
	if(this.isParameterDefined(tempArguments.CSS)){
...
biTurbo.prototype.createTween = function (name , duration , easing){
	easing=this.defaultParameter(easing,1);
	var temp_easing="";
	switch(easing){
		case 0: // linear
		default:
...
biTurbo.prototype.createDivElement = function (name , fileNameString , style, excludeFromElements){
	...
	var tempInsertionPoint=document.getElementById(this.insertionPoint)
	var temp_divEnding
	...
	tempInsertionPoint.innerHTML+='<div id="'+ name +'" class="' +  name +'">' + temp_divEnding;
		console.log("createDivElement: "+name+" : "+excludeFromElements)
...
biTurbo.prototype.createBorder = function (color,borderWidth){
	...
	//writeClass(".border_style" , tempStyle)
	//applyClass("border_frame", "border_style")
}
...
biTurbo.prototype.nextFrameInTimeLine = function (delay){
	...
	console.log("nextFrameInTimeLine :" + delay)
	console.log(biTurboProps.clicked + " " + this.currentFrameIndex + " " + this.timeLine.length )
	var context=this;
	setTimeout(function(){
...
biTurbo.prototype.setBTN = function (element,userClickTagFunctionName,endFrame){
	var element=this.getDomElement(element);
	var temp_estimated_endFrame=this.timeLine.length-1
	var endFrame=this.defaultParameter(endFrame,this.timeLine[temp_estimated_endFrame]);
	var context=this;
	element.addEventListener("click", function(){context.clickTag(userClickTagFunctionName,endFrame)})
...
biTurbo.prototype.checkLoaded = function (){
	...
	var context=this;
		setTimeout(function(){
			...
			var temp_loaded=0;
			var element;
			for(i=0 ; i<context.imagesToLoad.length ; i++){
				element=context.getDomElement(context.imagesToLoad[i])
				if(element.complete === true){
					temp_loaded++;
				}
			}
			context.a_print("-- \n imagesLoaded "+temp_loaded+'/'+context.imagesToLoad.length)
			if(temp_loaded === context.imagesToLoad.length){
...
biTurbo.create = function () { return Object.create(this.prototype) };
