...
biTurbo.proto.createSpriteSheet = function (ssObject){
	...
	var tempPositionY = 0;
	var tempFrame = 0;
	var temp_nameContentDiv = ssObject.htmlElement +"_content";
	var temp_containerStyle = "width : "+ ssObject.width + "px; height : " + ssObject.height + "px;  overflow:hidden";
	var back_insertionPoint = this.insertionPoint;
	ssObject.framesArray = [];

	this.createDivElement(ssObject.htmlElement,"", temp_containerStyle,false);
	this.insertionPoint = ssObject.htmlElement;
	this.createDivElement(temp_nameContentDiv,ssObject.source,"",true);
	this.insertionPoint = back_insertionPoint;
	
	for (tempFrame = 0 ; tempFrame < ssObject.totalFrames ; tempFrame++){
		tempCol++;
		if(tempCol>ssObject.col-1){
			tempRow ++;
			tempCol -= ssObject.col;
		}
		tempPositionX=-tempCol*ssObject.width;
		tempPositionY=-tempRow*ssObject.height;
		ssObject.framesArray.push([tempPositionX , tempPositionY]);
		if(ssObject.verbose === true){
			this.a_print("Spritesheet building frame table: "+ tempFrame + "  x: " + ssObject.framesArray[tempFrame][0]+ " y: "+ ssObject.framesArray[tempFrame][1] );
		}
	}
};

biTurbo.proto.nextFrameSpriteSheet = function (context,ssObject,direction,functionName){
	var temp_transformMatrixStyle = "";
	var temp_nameContentDiv = ssObject.htmlElement+"_content";
	if(direction>0){
		if(ssObject.currentFrame<ssObject.framesArray.length){
			ssObject.currentFrame++;
		}else{
			ssObject.currentFrame = 1;
		}
	}else{
		if(ssObject.currentFrame>1){
			ssObject.currentFrame--;
		}else{
	...
	if(ssObject.framesToPlay>0){
		ssObject.positionX = ssObject.framesArray[ssObject.currentFrame-1][0];
		ssObject.positionY = ssObject.framesArray[ssObject.currentFrame-1][1];
...
biTurbo.proto.playSpriteSheet = function (ssObject,delay,initFrame,framesToPlay, direction, functionName){
	...
	setTimeout(function(direction){
		ssObject.temporalInterval=setInterval (context.nextFrameSpriteSheet , ssObject.frameDuration , context , ssObject , direction , functionName);
	},delay*1000);
};

biTurbo.proto.stopSpriteSheet = function (context,ssObject,functionName){
	clearInterval(ssObject.temporalInterval);
	//functionName = this.defaultParameter(functionName,"no");

	if(functionName !== undefined){
		context.a_print("should be executed function "+functionName);
		window[functionName]();
	}
};
...
biTurbo.proto.preload = function (autoPlay){
	var context = this;
	var eventBannerLoadedId = "loadedEnough_"+this.banner_id;
	this.checkLoaded(context);
	if(autoPlay === true){
		addEventListener(eventBannerLoadedId,function(Event){
			context.play(context);
		});
	}
};

biTurbo.proto.play = function (context , force){
	context = this.defaultParameter(context , this);
	context.a_print(context.banner_id + " : is ready to play : " + context.readyToPlay + "  forced play : "+force);
...
biTurbo.proto.checkLoaded = function (context){
	var eventBannerLoadedId = "loadedEnough_"+context.banner_id;
	var readyEvent;
	if(typeof(Event) === "function") {
		context.readyEvent = new Event(eventBannerLoadedId);
	}else{ //IE fix
    	context.readyEvent = document.createEvent('Event');
    	context.readyEvent.initEvent(eventBannerLoadedId, true, true);
	}
	if(this.limitToWaitPreload > 0){
		setTimeout(function(){
			...
			if(temp_loaded === imagesToLoadCount){
				...
				context.a_print("-- \n preload complete : "+eventBannerLoadedId);
	...
	context.a_print("-- \n preloader timeOut : "+eventBannerLoadedId);
	context.readyToPlay = true;
	this.dispatchEvent(context.readyEvent);
	}
};
